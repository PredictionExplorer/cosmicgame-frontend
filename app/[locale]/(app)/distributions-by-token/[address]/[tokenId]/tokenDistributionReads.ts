 ...
 * for the anchor-holder (`useAnchorDistributionsByUserByTokenDetails`) and the
 * NFT itself (`useCSTInfo`: its name, cycle and the plate's seed), keyed like
 * the client hooks. The first HTML is then the page as the browser shows it:
 ...
 * read under the e2e harness, whose seeds are off.
 */
export const readTokenDistributionSeeds = cache(
  async (address: string, tokenId: number): Promise<QuerySeedEntry[]> => {
    if (seedsDisabled() || !Number.isSafeInteger(tokenId) || tokenId < 0) return [];
    const [deposits, token] = await Promise.all([
