...
import {
  ...
  type RoleKey,
} from '@/components/home/experimental/SpecialAllocationRecipients';
import { GestureStatus } from '@/components/home/experimental/GestureStatus';
import { InfoTooltip } from '@/components/ui/info-tooltip';
import { CyclePhaseGuide } from '@/components/home/experimental/CyclePhaseGuide';
import { GestureForm } from '@/components/home/experimental/GestureForm';
...
import { ADVANCED_SIDE_MEDIA_QUERY } from '@/components/home/experimental/GestureForm';
import { GestureAdvancedPanel } from '@/components/home/experimental/GestureAdvancedFields';
import { DeckArtCard } from '@/components/home/experimental/DeckArtCard';
import { DeckMiniBar } from '@/components/home/experimental/DeckMiniBar';
...
const ExperimentalHomePage = ({
  ...
  // The mini-bar appears only after the Deck has passed above the viewport.
  // Merely being below the initial viewport must not activate it over the
  // artwork hero (the reference PR's documented scroll-zero overlap).
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (!entry) {
          setDeckOutOfView(false);
          return;
        }
        const rootTop = entry.rootBounds?.top ?? 0;
        const deckBottom = entry.boundingClientRect?.bottom ?? Number.POSITIVE_INFINITY;
        setDeckOutOfView(!entry.isIntersecting && deckBottom <= rootTop);
      },
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link
              ...
              className="liquid-glass-control inline-flex items-center gap-1.5 rounded-full border border-primary/25 bg-primary/[0.06] px-3.5 py-2 text-xs font-semibold text-primary transition-colors hover:border-primary/45 hover:text-white"
            >
        ...
        <div className="fixed inset-x-3 bottom-3 z-40 sm:hidden">
          {account ? (
            <Button
              ...
              className="liquid-glass-cta h-12 w-full rounded-full shadow-[0_20px_70px_-30px_rgb(var(--aurora-cyan-rgb)/1)]"
            >
              {submitLabel}
            </Button>
          ) : (
