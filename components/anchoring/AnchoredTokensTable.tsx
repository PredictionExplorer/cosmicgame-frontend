...
type AnchoredRow = RandomWalkRow | CosmicSignatureRow;
...
interface AnchoredTokenRowProps {
  row: AnchoredRow;
  isRandomWalk: boolean;
  isItemSelected: boolean;
  accumulatedRewards: number;
  onRowClick: (id: number) => void;
  onReleaseSingle: (id: number) => void;
}

const AnchoredTokenRow = ({
  row,
  isRandomWalk,
  isItemSelected,
  accumulatedRewards,
  ...
  const anchorTimeStamp = isRandomWalk
      ...
      <TablePrimaryCell align="center">
        <Button
          ...
          onClick={async (e) => {
            ...
            setProcessing(true);
            try {
              await onReleaseSingle(anchorActionId);
            } finally {
              setProcessing(false);
            }
          }}
...
interface AnchoredTokensTableProps {
  list: AnchoredRow[];
...
export const AnchoredTokensTable = ({
  list,
  handleUnstake,
  handleUnstakeMany,
  IsRwalk,
  ...
  const onReleaseManyClick = async () => {
    setProcessing(true);
    try {
  ...
  const onReleaseSingle = async (actionId: number) => {
    ...
    <>
            ...
            {sortedList.slice((page - 1) * perPage, page * perPage).map((row) => {
              ...
              return (
                <AnchoredTokenRow
                  ...
                  isItemSelected={isSelected(anchorActionId)}
                  onRowClick={handleRowClick}
                  onReleaseSingle={onReleaseSingle}
                />
              );
            })}
          </Tbody>
      ...
      {selectedIds.length > 1 && (
        <div className="flex justify-end mt-4">
          <Button variant="text" disabled={processing} onClick={onReleaseManyClick}>
            {processing ? (
              <span className="flex items-center gap-1">
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                Releasing...
