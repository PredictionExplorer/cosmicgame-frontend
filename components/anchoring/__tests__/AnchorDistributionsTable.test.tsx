...
describe('AnchorDistributionsTable', () => {
  ...
  it('renders only first page of results (perPage=5)', () => {
    ...
    render(<AnchorDistributionsTable list={list} address={ADDRESS} />);
    expect(screen.getByText('104')).toBeInTheDocument();
    expect(screen.queryByText('105')).not.toBeInTheDocument();
  });

  it('navigates to rewards-by-token page on row click', () => {
    render(<AnchorDistributionsTable list={[createRow({ TokenId: 42 })]} address={ADDRESS} />);
  ...
  it('has no accessibility violations', async () => {
    const { container } = render(<AnchorDistributionsTable list={[]} address={ADDRESS} />);
