...
describe('BiddingStatus', () => {
  ...
  it('shows cycle opening when no performance has started', () => {
    render(
      <BiddingStatus
        {...baseProps}
    ...
    expect(screen.getByText('Cycle opening')).toBeInTheDocument();
    expect(screen.getByText(/ETH Calibration Window/)).toBeInTheDocument();
  });

  it('shows cycle label when round number exists but performance not started', () => {
    render(
      <BiddingStatus
        {...baseProps}
