 ...
 * (a ledger link, a chat row near the bottom edge) receives focus and stays
 * hidden. When keyboard focus lands under the dock, the page lifts by the
 * overlap. A tap never moves the page, and a dock that has stepped aside
 * (`data-state="aside"`, translated off screen) is ignored.
 */
export function useFocusClearOfDock(enabled: boolean): void {
  useEffect(() => {
    ...
    const handleFocusIn = (event: FocusEvent) => {
      ...
      if (!dock || dock.contains(target) || dock.dataset.state === 'aside') return;
