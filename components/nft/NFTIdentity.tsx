...
/**
 * NFTIdentity — the detail page's wall label, by the rule every art surface
 * follows (`components/ui/signature-label`): the trail to the gallery, the
 * name (or "Signature #000025", its number in Inter figures at display size),
 * the token number in mono with its structure and palette, then the
      ...
      >
        <WallLabelMeta
          ...
          items={label.meta({
            tokenId,
            name,
            structure: entry?.structure ? valueLabel('structure', entry.structure) : null,
            palette: entry?.palette ? valueLabel('palette', entry.palette) : null,
          })}
        />
      </PageHeader>
