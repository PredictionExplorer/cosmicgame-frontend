...
/**
 * The Cosmic Signature NFTs an address holds, as the gallery hangs them:
 * each Signature on its black plate at the native ratio, nothing over the
 * art, and a wall label beneath (name or number, token number, cycle).
 * Newest first; the first two rows show until "Show all".
 */
export function ProfileArtworks({ tokens, loading }: ProfileArtworksProps) {
  ...
  const ordered = [...tokens].sort((a, b) => b.TokenId - a.TokenId);
