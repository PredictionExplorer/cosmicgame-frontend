...
/**
 * Background utility for each track's bar segment and legend swatch: the named track tokens
 * (styles/themes.css → data series), fixed hues in every palette, so a track reads the same on
 * every page and chart. The palette primary and secondary are close lilacs in Midnight and
 * Nebula, so they cannot tell two tracks apart; red is never a track.
 */
