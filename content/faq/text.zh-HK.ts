  ...
  'getting-started': {
    ...
    items: {
      ...
      'what-type-of-gestures-are-available': {
        question: '可以用哪些方式落筆？',
        answer:
          '落筆可使用 ETH 或 CST 代幣（ERC-20）。每個週期的首筆落筆必須使用 ETH；之後可以自由交替使用 ETH 與 CST。ETH 落筆還可附加一枚 Random Walk NFT，使 ETH 落筆價格降低 50%。Cosmic Signature NFT（ERC-721）用於分配與錨定，不能用來支付落筆。CST 落筆使用獨立的校準窗口：窗口運行期間，CST 落筆價格會逐步下降；每筆 ETH 或 CST 落筆又會改變窗口本身的時長。',
      },
      ...
      'how-can-i-get-involved': {
        question: '有哪些參與方式？',
        answer:
          '你可以在演繹週期中落筆，也可以將自己項目的 NFT 貢獻給協議，供參與者隨落筆附加。加入 Discord，即可認識其他參與者。',
      },
  ...
  'allocations-and-rewards': {
    ...
    items: {
      ...
      'what-rewards-per-bid': {
        question: '每次落筆會帶來什麼？',
        answer: `每筆落筆都會記錄 1 次週期末星選資格，更新堅守窗口對堅守冠軍和時之勇士軌道的貢獻，並可能銘刻參與 CST。參與 CST 按${isV3Mechanics ? '線性' : '平方根'}公式計算：${cstRewardFacts.formula}。簡單來說，距上一筆落筆越久，數量越多${isV3Mechanics ? '，且按恆定速率增長' : '，但增長速度會逐漸放緩'}。極短間隔可能得到 0 CST；較長的靜默期則可能產生更多 CST 銘刻。`,
      },
      'how-does-the-stellarSelection-work': {
        question: '星選如何運作？',
        answer: `每筆落筆都會記錄 1 次星選資格。每個週期結束時，智能合約會從資格池中進行程序化隨機選擇：${protocolFacts.ethStellarSelectionRecipients} 次選擇共同分得週期儲備中 ${ethDistributionFacts.stellarSelectionEthPercentage}% 的 ETH；${protocolFacts.nftStellarSelectionRecipients} 次選擇各獲得 ${protocolFacts.specialAllocationCst.toLocaleString('zh-HK')} CST 與 1 枚 Cosmic Signature NFT；已錨定 Random Walk NFT 中另有 ${protocolFacts.anchoredRwlkNftSelectionRecipients} 次選擇，也各獲得 ${protocolFacts.specialAllocationCst.toLocaleString('zh-HK')} CST 與 1 枚 Cosmic Signature NFT。選擇採用放回方式，同一地址在同一週期內可能被選中多次。${isV3Mechanics ? 'ETH 星選按每筆落筆當時的未折扣 ETH 成本加權，成本越高，被選中的概率按比例越大；NFT 星選仍為每筆落筆 1 次資格。' : '落筆次數越多，被選中的頻次也會增加。'}`,
      },
      'how-random-selection-works': {
        question: '程序化隨機選擇是怎樣完成的？',
        answer: `星選在週期收官時使用鏈上隨機源，包括 Arbitrum 提供的區塊上下文與備用熵源。${isV3Mechanics ? '參與者 ETH 星選按成本加權：每次資格攜帶該筆落筆當時的未折扣 ETH 成本，成本越高被選中的概率按比例越大；NFT 星選仍按資格計數（每筆落筆 1 次資格）。' : '參與者星選按資格加權：每筆落筆增加 1 次資格，因此落筆越多，被選中的頻次越高。'}錨定 NFT 星選則是獨立機制，依據已錨定 Random Walk NFT 的資格，而不是參與者落筆資格池。`,
      },
      'how-do-i-claim-my-allocation': {
        question: '成為獲配者後，如何取回分配？',
        answer: `獲配者可透過應用程式與協議合約取回分配。週期收官倒數結束後，收官之筆參與者享有 ${protocolFacts.finalGestureExclusivityHours} 小時的專屬時間，可完成週期收官並取回簽名分配。此後進入公開收官窗口：任何人都可發起收官交易，智能合約會把實際完成收官的人視為週期受益方——收官者將獲得整份簽名分配，包括 ETH 份額、${protocolFacts.specialAllocationCst.toLocaleString('zh-HK')} CST 銘刻、Cosmic Signature NFT，以及對已附加資產的優先權。次級 ETH、已附加代幣與已附加 NFT 會由分配錢包託管，並採用另一項取回超時設定，目前預設為 ${protocolFacts.secondaryRetrievalTimeoutWeeks} 週；超時後，合約允許任何人為自己取回仍未取回的分配。請及時處理。`,
      },
      'how-does-anchoring-work': {
        question: '錨定如何運作？',
        answer: `Cosmic Signature NFT 可錨定至協議，以獲得 ETH 錨定派發：每個已收官週期會劃出週期儲備的 ${ethDistributionFacts.anchorDistributionPercentage}%，按當時已錨定的 Cosmic Signature NFT 數量平均分配；累積的 ETH 會在解錨時派發。Random Walk NFT 也可以錨定，但只用於取得錨定 NFT 星選資格——被選中的錨定者會獲得 CST 與 Cosmic Signature NFT，而不是 ETH。還需注意兩條規則：每枚 NFT 永遠只能錨定一次（解錨後不可再次錨定）；若週期收官時沒有任何 Cosmic Signature NFT 處於錨定狀態，該週期的 ${ethDistributionFacts.anchorDistributionPercentage}% 會留在週期儲備中。CST（ERC-20）不能錨定。可從帳戶選單進入「我的錨定」頁面管理錨定。`,
      },
      'what-are-marketing-rewards': {
        question: '什麼是推廣儲備？',
        answer: `幫助推廣協議可獲得 CST 代幣（ERC-20）。推廣儲備每個週期銘刻 ${protocolFacts.outreachReserveCst.toLocaleString('zh-HK')} CST，並將其發放給生態貢獻者。具體方式可在 Discord 聯絡推廣託管人。`,
      },
      'how-many-nfts-minted': {
        question: '每個週期會銘刻多少枚 Cosmic Signature NFT？',
        answer: `絕大多數週期會銘刻 ${nftAllocationFacts.typicalNftsPerCycle} 枚 Cosmic Signature NFT：簽名分配獲配者 ${nftAllocationFacts.mainPrizeNftsWord.zhHant} 枚；CST 收官之筆獲配者、堅守冠軍與時之勇士各 1 枚；參與者 NFT 星選獲配者共 ${protocolFacts.nftStellarSelectionRecipients} 枚；透過錨定 NFT 星選選出的 Random Walk NFT 錨定者共 ${protocolFacts.anchoredRwlkNftSelectionRecipients} 枚。這 ${nftAllocationFacts.nftBearingAllocations} 份 NFT 分配還會各附帶 ${protocolFacts.specialAllocationCst.toLocaleString('zh-HK')} CST。若某週期沒有 CST 落筆或沒有已錨定的 Random Walk NFT，對應的銘刻便會在該週期跳過。`,
      },
      ...
      'who-receives-10-percent': {
        question: '週期儲備中的公共物品分配會發給誰？',
        answer: `週期收官時，週期儲備的 ${ethDistributionFacts.publicGoodsPercentage}% 會轉入公共物品金庫；此後任何人都可將金庫餘額轉給已配置的公共物品受益方。目前受益方為 Protocol Guild，這是支持 170 多位以太坊核心貢獻者的集體資助機制。目前，受益方地址由協議所有者設定；待協議所有權交由宇宙議會後，設計目標是由議會決定受益方。`,
      },
    },
  },
  'game-mechanics': {
    ...
    items: {
      ...
      'how-is-participation-cst-calculated': {
        ...
        answer: isV3Mechanics
          ? `參與 CST 隨距上一筆落筆的時間線性累積：${cstRewardFacts.formula}。按協議上線時恰為 1 小時的時間增量計算，速率約為每分鐘 ${protocolFacts.v3.dynamicCstRewardPerMinuteAtLaunch} CST，示例約為：${cstRewardFacts.examples.map((example) => `${ELAPSED_ZH_HK[example.elapsed] ?? example.elapsed}後為 ${example.cst} CST`).join('、')}。每個週期收官後，時間增量會增長 ${protocolFacts.cycleTimeIncrementIncreasePercentPerCycle}%，因此即時數量會隨時間逐漸略低於這些示例。落筆實際成交時，應以應用程式中的即時預覽和合約計算為準。`
          : `參與 CST 按距上一筆落筆的時間，以平方根公式計算：${cstRewardFacts.formula}。採用平方根，是為了讓較長的靜默期獲得更多 CST，同時避免數量永遠線性增長。按協議上線時恰為 1 小時的時間增量計算，示例約為：${cstRewardFacts.examples.map((example) => `${ELAPSED_ZH_HK[example.elapsed] ?? example.elapsed}後為 ${example.cst} CST`).join('、')}。每個週期收官後，時間增量會增長 ${protocolFacts.cycleTimeIncrementIncreasePercentPerCycle}%，因此即時數量會隨時間逐漸略低於這些示例。落筆實際成交時，應以應用程式中的即時預覽和合約計算為準。`,
      },
      ...
      'how-cst-calibration-window-changes': {
        ...
        answer: `每筆 ETH 或 CST 落筆都會更新鏈上保存的 CST 校準窗口。CST 落筆會把窗口延長「目前時長 / ${protocolFacts.cstCalibrationWindowChangeDivisor}」，整數截斷前約為 +${protocolFacts.cstCalibrationWindowIncreasePercentPerCstGesture}%；ETH 落筆會把窗口縮短約「目前時長 / ${protocolFacts.cstCalibrationWindowChangeDivisor + 1}」，約為 -${protocolFacts.cstCalibrationWindowDecreasePercentPerEthGesture}%。窗口越短，CST 落筆價格下降越快；窗口越長，下降越慢。`,
      },
      'what-is-open-finalization-window': {
        question: '什麼是公開收官窗口？',
        answer: `週期收官倒數結束後，收官之筆參與者享有 ${protocolFacts.finalGestureExclusivityHours} 小時的專屬收官時間。若未在該窗口內完成收官，任何人都可發起收官交易，智能合約會把實際收官者設為週期受益方。收官者將獲得完整的簽名分配，包括 ETH 份額、${protocolFacts.specialAllocationCst.toLocaleString('zh-HK')} CST、Cosmic Signature NFT，以及對已附加資產的優先權。因此，收官之筆參與者應在專屬窗口結束前完成收官。即使該參與者離開，公開收官機制也能讓協議繼續運行。`,
      },
      ...
      'what-is-chrono-warrior': {
        question: '什麼是時之勇士？',
        answer: `時之勇士是在堅守冠軍位置上連續保持最久的參與者。堅守冠軍對應保持「最近一筆落筆者」身份最久的人，時之勇士則對應保持堅守冠軍身份最久的人。週期收官時，時之勇士將獲得週期儲備中 ${ethDistributionFacts.chronoWarriorEthPercentage}% 的 ETH、${protocolFacts.specialAllocationCst.toLocaleString('zh-HK')} CST 與 1 枚 Cosmic Signature NFT。`,
      },
      ...
      'how-time-increase-affects-game': {
        ...
        answer:
          '隨着每筆落筆增加的時間變長，週期的平均持續時間也會延長。這一變化循序漸進，既保持平穩的參與體驗，也限制長期範圍內 Cosmic Signature NFT 的總供應增長。',
      },
  ...
  'tokens-and-nfts': {
    ...
    items: {
      'what-are-cst-and-dao': {
        ...
        answer: `每筆落筆都可能銘刻 CST 代幣；CST 用於表達宇宙議會中的協調權重。議會在鏈上協調協議：CST 持有者可以提交協調提案，並表達支持或反對。要啟用權重，需先把 CST 委託給自己或其他地址。待合約所有權交由議會後，議會按設計將管理協議參數，包括決定哪個公共物品受益方獲得 ${ethDistributionFacts.publicGoodsPercentage}% 的分配；目前這些設定仍由協議所有者管理。`,
      },
      ...
      'what-makes-nfts-unique': {
        ...
        answer:
          'Cosmic Signature NFT 完全位於鏈上，並可持續獨立生成。每枚 NFT 都銘刻了智能合約隨機生成並保存的種子；圖像與影片由開源 Rust 流水線根據該種子渲染。種子決定三個天體的初始條件，從而為每枚 NFT 產生獨一無二的混沌軌跡。',
      },
      'how-are-nft-images-created': {
        question: 'NFT 圖像是怎樣生成的？',
        answer:
          '每枚 Cosmic Signature NFT 都以牛頓引力下的三體問題為主題。流水線模擬三個天體在引力作用下的運動，並在 380 至 700 納米範圍內以 64 個波長區間對軌跡進行光譜渲染，為每枚 NFT 生成獨特的混沌圖案。',
      },
      'significance-of-random-seed': {
        question: '為什麼每枚 NFT 都由鏈上種子生成？',
        answer:
          '基於種子的流水線可確保作品長期可復現。有些 NFT 項目的圖像依賴中心化伺服器，而每枚 Cosmic Signature NFT 的種子都儲存在 Arbitrum 上。任何人都能隨時使用開源 Rust 流水線獨立重新生成 NFT 圖像和影片，逐像素與原作一致。',
      },
      ...
      'where-to-buy-cosmic-signature-nfts': {
        question: '在哪裏可以買賣 Cosmic Signature NFT？',
        answer:
      ...
      'cosmic-signature-prediction-market': {
        ...
        answer:
          '有。Chaos Zero（https://chaoszero.com）是專為 Cosmic Signature 構建的預測市場。每個演繹週期都會提出一個問題：本週期收官時的落筆次數是否會超過上一週期？所有頭寸以 CST 計價，並按機制完全抵押；市場依據公開的鏈上落筆次數結算，不設所有者或管理密鑰。',
      },
  ...
  'arbitrum-and-technical': {
    ...
    items: {
      ...
      'arbitrum-security': {
        ...
        answer:
          'Arbitrum 是彙總網絡，而不是側鏈。每批交易都會發布回以太坊主網，因此 Arbitrum 的安全性錨定在以太坊本身：數據與爭議解決均位於 Layer 1。',
      },
      ...
      'existing-wallet-on-arbitrum': {
        ...
        answer:
          '可以。同一組私鑰可在兩個網絡上籤署交易，只需把 Arbitrum 添加到錢包的網絡列表即可。',
      },
      'view-tokens-on-arbitrum': {
        question: '如何查看 Arbitrum 上的 CST 與 Cosmic Signature NFT？',
        answer:
          '可以直接在 Cosmic Signature 網站查看，也可手動把合約地址添加至錢包。所有合約地址都公佈在「合約」頁面與社區 Discord 中。',
      },
      'trade-on-arbitrum': {
      ...
      'verify-bid-success': {
        ...
        answer:
          '成功的落筆會在 Arbitrum 上得到確認，並顯示在 Arbitrum 區塊瀏覽器 Arbiscan 中。將交易哈希粘貼至瀏覽器，即可驗證這筆落筆。',
      },
      'game-security': {
        question: '協議如何保障安全？',
        answer:
          'Cosmic Signature 公開合約地址、源代碼資源與驗證背景，便於社區獨立檢查協議行為。智能合約已由獨立安全機構 Hacken 完成審計，完整報告見「審計」頁面。',
      },
      'fees-involved': {
        question: '參與需要支付哪些費用？',
        answer:
  ...
  'trust-and-governance': {
    ...
    items: {
      'team-controls': {
        ...
        answer:
          '初期，團隊可以調整部分協議參數，例如落筆時間增量或分配軌道比例。這些權限透過智能合約的 Ownable 模式實現，並限定在週期間窗口：下一個週期一旦啟用（啟用發生在首筆落筆之前），核心協議參數便會鎖定，直至該週期收官。鎖定期間仍保留少量範圍更窄的權限：所有者可把週期啟用推遲至首筆落筆到來，也可隨時調整下一週期前的延遲，並隨時管理外圍合約（公共物品金庫受益方、NFT 元數據 URI 與分配錢包取回期限）。協議合約還可由所有者透過 UUPS 升級，但只能在週期間進行；目前部署的是已公開驗證的 V2 實現。',
      },
      ...
      'what-is-renounce-ownership': {
        ...
        answer:
          '放棄所有權是 Ownable 合約的一項函數，會永久把控制權從部署者地址移走。調用後，任何特權角色都無法再修改合約參數。',
      },
      ...
      'how-team-profits': {
        ...
        answer:
          '參與者落筆所支付的 ETH 不會進入任何團隊錢包。所有 ETH 都匯入週期儲備，並按各條分配軌道發放。團隊透過持有 Random Walk NFT 與協議間接保持利益一致；協議成功可能提升這些 NFT 的文化價值。團隊的主要動力是好奇心、創造力，以及為開源公共物品作出貢獻。',
      },
      'why-was-cs-created': {
        question: '為什麼要創建 Cosmic Signature？',
        answer:
          'Cosmic Signature 源於對混沌理論與三體問題無解析解特性的着迷。由鏈上種子生成獨特而確定的藝術，既引人入勝，也契合支持公共物品的協議理念。',
      },
      'what-if-team-disappears': {
        question: '如果團隊不再維護項目，會怎樣？',
        answer:
          '協議按可持續獨立運行的目標設計。種子儲存在鏈上，任何人都可以使用開源 Rust 流水線重新生成 NFT 圖像與影片。無論團隊狀態如何，每枚 Cosmic Signature NFT 都能持續訪問。',
      },
      'can-create-competing-site': {
        question: '可以復刻代碼並搭建自己的網站嗎？',
        answer:
          '當然可以。項目自有的合約、着色器、渲染器、頁面與文檔均採用 CC0 1.0，不保留任何權利。第三方依賴、字體與素材仍適用各自的授權條款；詳見 THIRD_PARTY_NOTICES.md。',
      },
      'donate-to-pot': {
        question: '不落筆也能向週期儲備貢獻 ETH 嗎？',
        answer:
          '可以。協議合約提供獨立於落筆的專用貢獻函數，可接收 ETH，也可附加備註並顯示在週期貢獻列表中。請使用應用程式內的貢獻流程，不要直接從錢包向協議地址轉帳：直接發送至協議地址的 ETH 會被處理為 ETH 落筆，而不是貢獻。詳情可透過 Discord 諮詢。',
      },
      'get-help': {
      ...
      'stay-updated': {
        ...
        answer: '關注官方社交媒體並加入 Discord 社區，即可獲取最新公告、協議協調提案與週期回顧。',
      },
    },
  },
} satisfies FAQText;
