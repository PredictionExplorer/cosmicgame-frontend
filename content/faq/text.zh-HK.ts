  ...
  'game-mechanics': {
    ...
    items: {
      ...
      'how-is-participation-cst-calculated': {
        ...
        answer: `參與 CST 按距上一筆落筆的時間，以平方根公式計算：${protocolFacts.dynamicCstRewardFormula}。採用平方根，是為了讓較長的靜默期獲得更多 CST，同時避免數量永遠線性增長。\n\n按協議上線時恰為 1 小時的時間增量計算，示例約為：${protocolFacts.dynamicCstRewardExamples.map((example) => `${ELAPSED_ZH_HK[example.elapsed]}後為 ${example.cst} CST`).join('、')}。每個週期收官後，時間增量會增長 ${protocolFacts.cycleTimeIncrementIncreasePercentPerCycle}%，因此即時數量會隨時間逐漸略低於這些示例。落筆實際成交時，應以應用程式中的即時預覽和合約計算為準。`,
      },
  ...
  'trust-and-governance': {
    ...
    items: {
      'team-controls': {
        ...
        answer:
          '初期，團隊可以調整部分協議參數，例如落筆時間增量或分配軌道比例。這些權限透過智能合約的 Ownable 模式實現，並限定在週期間窗口：下一個週期一旦啟用（啟用發生在首筆落筆之前），核心協議參數便會鎖定，直至該週期收官。\n\n鎖定期間仍保留少量範圍更窄的權限：所有者可把週期啟用推遲至首筆落筆到來，也可隨時調整下一週期前的延遲，並隨時管理外圍合約（公共物品金庫受益方、NFT 元數據 URI 與分配錢包取回期限）。\n\n協議合約還可由所有者透過 UUPS 升級，但只能在週期間進行；目前部署的是已公開驗證的 V2 實現。',
      },
