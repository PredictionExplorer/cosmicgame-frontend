 ...
 * into a copy module).
 */
const cst = formatCount(protocolFacts.specialAllocationCst, 'zh-HK');
const outreachCst = formatCount(protocolFacts.outreachReserveCst, 'zh-HK');
  ...
  'allocations-and-rewards': {
    ...
    items: {
      ...
      'how-does-the-stellarSelection-work': {
        question: '星選如何運作？',
        answer: `每筆落筆都會記錄 1 次星選資格。每個週期結束時，智能合約會從資格池中進行程序化隨機選擇：${protocolFacts.ethStellarSelectionRecipients} 次選擇共同分得週期儲備中 ${protocolFacts.stellarSelectionEthPercentage}% 的 ETH；${protocolFacts.nftStellarSelectionRecipients} 次選擇各獲得 ${cst} CST 與 1 枚 Cosmic Signature NFT；已錨定 Random Walk NFT 中另有 ${protocolFacts.anchoredRwlkNftSelectionRecipients} 次選擇，也各獲得 ${cst} CST 與 1 枚 Cosmic Signature NFT。選擇採用放回方式，同一地址在同一週期內可能被選中多次。落筆次數越多，被選中的頻次也會增加。`,
      },
  ...
  'game-mechanics': {
    ...
    items: {
      ...
      'what-is-open-finalization-window': {
        question: '什麼是公開收官窗口？',
        answer: `週期收官倒數結束後，收官之筆參與者享有 ${protocolFacts.finalGestureExclusivityHours} 小時的專屬收官時間。若未在該窗口內完成收官，任何人都可發起收官交易，智能合約會把實際收官者設為週期受益方。收官者將獲得完整的簽名分配，包括 ETH 份額、${cst} CST、Cosmic Signature NFT，以及對已附加資產的優先權。因此，收官之筆參與者應在專屬窗口結束前完成收官。即使該參與者離開，公開收官機制也能讓協議繼續運行。`,
      },
      ...
      'why-time-per-bid-increases': {
        ...
        answer:
          '這一機制決定 Cosmic Signature NFT 的長期銘刻節奏：週期越長，同一段時間內銘刻的新 NFT 越少。',
      },
  ...
  'tokens-and-nfts': {
    ...
    items: {
      ...
      'impact-of-limiting-nfts': {
        question: '週期長短如何影響新 NFT 的銘刻數量？',
        answer:
          '落筆時間增量逐步增長，通常會使週期變長，因此同一段時間內銘刻的 NFT 會減少。每一枚 Cosmic Signature NFT 都記錄了協議歷史的一部分，總量沒有固定上限。',
      },
