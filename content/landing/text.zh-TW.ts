...
export const landingTextZhTw = {
  ...
  hero: {
    ...
    subhead:
      '參與 Arbitrum 上持續生長的藝術協議。每次落筆都在形塑下一份簽名；每個週期都會將儲備分配給參與者及以太坊的公共基礎設施。',
    biologyDisclaimer:
  ...
  anchoring: {
    ...
    body: `每個週期，${ethDistributionFacts.anchorDistributionPercentage}% 的 ETH 週期儲備會用於錨定配發。已錨定的 Cosmic Signature NFT 會按比例累積相應份額，解錨時即可取回。每枚 NFT 僅可錨定一次，但可隨時解錨；一旦解錨，便永久失去再次錨定的資格。已錨定的 Random Walk NFT 可獲得錨定 NFT 星選資格；入選錨定者將獲配 ${protocolFacts.specialAllocationCst.toLocaleString('zh-TW')} CST 與 1 枚 Cosmic Signature NFT（不含 ETH）。`,
    bullets: [
  ...
  faq: {
    ...
    items: [
      ...
      {
        question: '公共財具體指什麼？',
        answer: `每個週期會將 ETH 儲備的 ${ethDistributionFacts.publicGoodsPercentage}% 轉撥至公共財地址，目前為 Protocol Guild。Protocol Guild 是為 170 多位以太坊核心貢獻者提供資助的集體機制。這是向公共財地址轉撥 ETH，並非美國稅法意義上的慈善捐贈；Cosmic Signature 不對其稅務處理作任何陳述。`,
      },
      // lexicon-allow-end
      {
