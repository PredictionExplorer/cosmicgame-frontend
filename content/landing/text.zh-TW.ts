...
export const landingTextZhTw = {
  meta: {
    ...
    description: `Cosmic Signature 是運作於 Arbitrum 的程序化鏈上藝術協議。參與者在每個演繹週期中落筆；週期收官後，新的簽名作品隨之銘刻，週期儲備依規則分配至各條軌道，其中 ${protocolFacts.publicGoodsPercentage}% 轉撥給以太坊核心貢獻者。`,
    keywords: [
      'Cosmic Signature',
      '程序化藝術協議',
      '鏈上藝術',
  ...
  hero: {
    ...
    subhead: `每一次落筆以 ETH 或 CST 完成，都會延長週期的收官倒數。倒數歸零即可收官：為獲配者銘刻新的簽名作品，並將週期儲備分配到各條軌道，其中 ${protocolFacts.publicGoodsPercentage}% 轉撥給以太坊核心貢獻者。`,
  ...
  cycle: {
    ...
    steps: {
      gesture: {
        title: '落筆',
        body: 'ETH 或 CST 皆可參與。每一筆都記錄在鏈上，並計入本週期的星選資格。',
      },
      extend: {
        title: '延長倒數',
  ...
  anchoring: {
    ...
    body: `將 Cosmic Signature NFT 錨定至協議後，它每個週期都會按比例累積 ${protocolFacts.anchorDistributionPercentage}% 錨定配發中的份額，解錨時即可取回。可隨時解錨，但每枚 NFT 僅可錨定一次。`,
    bullets: [
      '每個週期累積 ETH，解錨時取回',
      '無固定期限、無罰則；每枚 NFT 解錨後不可再錨定',
      '姊妹系列 Random Walk NFT 也可以錨定',
      `已錨定的 Random Walk NFT 可獲得錨定 NFT 星選資格：${protocolFacts.specialAllocationCst.toLocaleString('zh-TW')} CST 與 1 枚 Cosmic Signature NFT，不含 ETH`,
    ],
    ctaLabel: '前往應用程式錨定',
  },
  ...
  faq: {
    ...
    items: [
      {
        question: '參與者實際要做什麼？',
        answer: `你可以落筆。每一筆都是使用 ETH 或 CST 發起的鏈上交易，會延長收官倒數、計入一次星選資格，還可能銘刻參與 CST，並共同塑造這一週期的簽名。你還可以將 Cosmic Signature NFT 錨定至協議，使其按比例參與錨定配發；持有至少 ${protocolFacts.councilProposalThresholdCst} CST 時，也可以透過宇宙議會提交協調提案。`,
      },
      {
      ...
      {
        question: '公共財具體指什麼？',
        answer: `每個週期會將 ETH 儲備的 ${protocolFacts.publicGoodsPercentage}% 轉撥至公共財地址，目前為 Protocol Guild。Protocol Guild 是為 170 多位以太坊核心貢獻者提供資助的集體機制。這是向公共財地址轉撥 ETH，並非美國稅法意義上的慈善捐贈；Cosmic Signature 不對其稅務處理作任何陳述。`,
      },
      // lexicon-allow-end
      // lexicon-allow-start: 明確否認彩票、賭場、賭博、莊家、荷官及賭注類別。
      {
        ...
        answer:
          '不是。Cosmic Signature 是程序化鏈上藝術協議。參與者在演繹週期中落筆；週期收官後，協議將儲備依規則分配至各條分配軌道。這裡沒有莊家，沒有荷官，也沒有賭注。分配所表彰的是堅守、時機與參與。唯一帶有隨機性的分配軌道——星選——是協議層面的程序化分配。',
      },
      // lexicon-allow-end
