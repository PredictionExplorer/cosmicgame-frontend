...
const count = (value: number) => formatCount(value, 'zh-TW');
...
export const auditsCopyZhTw: AuditsCopy = {
  ...
  audit: {
    ...
    paragraphs: [
      ...
      `除人工審查外，Hacken 還對 ${count(invariants.tested)} 項系統不變量進行了模糊測試，例如協議持有的 ETH 總額必須等於存入減去取回。全部 ${count(invariants.held)} 項不變量在 ${count(invariants.runs)} 次執行中均保持成立。`,
    ],
  },
  analysis: {
    heading: '形式化驗證與分析',
