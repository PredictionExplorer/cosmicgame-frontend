...
export const basicQuestionsTextKo = {
  ...
  'what-is-a-gesture': {
    ...
    explanation:
      '제스처는 사이클을 진행시키는 기본 참여 행위입니다. 제스처 하나하나가 ETH 또는 CST를 담고, 사이클 마감 시각을 뒤로 밀고, 사이클의 별빛 선정에 자격 하나를 기록하며, 참여 CST를 각인할 수 있습니다. 손으로 그리는 것은 아무것도 없습니다. 작품은 마감 시점에 시드에서 계산됩니다.',
    referenceLabel: '백서 §4 — 제스처',
  },
  'two-currencies': {
    prompt: '제스처 비용으로 사용할 수 있는 통화는 무엇인가요?',
    options: {
  ...
  'sleepy-beneficiary': {
    ...
    explanation: `최종 제스처 참여자는 ${protocolFacts.finalGestureExclusivityHours}시간 동안 이 권리를 독점합니다. 그 뒤에는 공개 마감 구간이 시작됩니다. 누구든 마감할 수 있고, 컨트랙트는 마감한 사람을 사이클 수령자로 간주하여 그 역할에 딸린 모든 것을 넘깁니다. 이 규칙은 최종 참여자가 마감하지 않아도 프로토콜을 계속 진행할 수 있게 합니다.`,
    funFact:
  ...
  'compounding-reserve': {
    prompt: '배분 뒤에 남은 사이클 준비금은 어떻게 되나요?',
    options: {
      a: `모든 사이클 준비금의 약 ${protocolFacts.compoundingReservePercentage}%는 다음 사이클로 이월됩니다.`,
      b: '팀이 회수한 뒤 다음 사이클 시작 전에 다시 채웁니다.',
      c: '다음 사이클에서 각인되는 새 ETH로 대체됩니다.',
      d: '우주 평의회가 선택한 지갑으로 전달됩니다.',
    },
    explanation:
      '다섯 ETH 배분 경로에 준비금의 절반이 배분되고, 나머지는 다음 사이클로 이월됩니다. 프로토콜이 새로운 ETH를 만드는 것은 아닙니다. 준비금이 매번 커지는 것은 아니며, 규모는 참여 상황과 배분에 따라 달라집니다.',
    referenceLabel: '백서 §5.1 — 마감 시 배분',
  },
  'art-engine': {
  ...
  'cst-supply-origin': {
    ...
    options: {
      a: '공급량은 0에서 시작하며, 정해진 규칙에 따라 프로토콜 컨트랙트만 CST를 각인할 수 있습니다.',
      b: '출시 때 팀을 위해 큰 몫이 만들어졌습니다.',
      c: '출시 전 초기 지갑에 무상으로 나누어졌습니다.',
      d: '누구든 토큰 컨트랙트를 호출해 CST를 각인할 수 있습니다.',
    },
    explanation:
      'CST 공급량은 0에서 시작했으며, 각인과 소각은 프로토콜 컨트랙트만 지시할 수 있습니다. 참여 CST, 공로 CST, 홍보 준비금 배분 등 정해진 규칙을 통해 새 CST가 각인됩니다.',
  ...
  'cst-on-spend': {
    ...
    explanation:
      '제스처 비용으로 사용한 CST는 전액 소각되어 공급량에서 제외됩니다. 이 CST가 준비금이나 다른 지갑으로 전달되는 것은 아닙니다.',
  ...
  'anchor-once-ever': {
    prompt: 'Mira가 앵커링된 NFT를 해제합니다. 다음 달에 그 NFT를 다시 앵커링할 수 있나요?',
    options: {
  ...
  'who-runs-cycles': {
    ...
    options: {
      a: '검증된 컨트랙트에 정해진 배분 규칙이 마감 시 실행됩니다.',
