 ...
 */
export const basicQuestionsTextZhHk = {
  'what-is-cosmic-signature': {
    prompt: '朋友問你：Cosmic Signature 到底是什麼？哪個回答是對的？',
    options: {
      a: '部署在 Arbitrum 上的程序化鏈上藝術協議，以一個個限時的演繹週期運轉。',
      b: '一個把文字提示詞變成太空圖片的 AI 圖像服務。',
      c: '生物學家使用的癌症突變特徵數據庫。',
      d: '一個預測 NFT 系列價格的服務。',
    },
    explanation:
      'Cosmic Signature 是程序化藝術協議：限時的演繹週期在一次次落筆中延展，收官時銘刻確定性的三體藝術作品。整條管線沒有任何 AI 參與——作品是由種子驅動的物理計算，與文字提示詞式的圖像服務恰好相反。',
    funFact:
      '這個名字與知名的癌症突變數據庫 COSMIC 撞了車。協議與它毫無關係——文檔中專門寫明瞭這一點。',
  ...
  'final-gesture-role': {
    ...
    explanation:
      '週期收官時間一到，收官之筆參與者即可收官，且起初這項權利專屬於他。落筆數量在這裏毫無作用：一筆恰到好處、堅持到最後的落筆，勝過此前的一百筆。',
    referenceLabel: '白皮書 §3.3 · 收官與公開收官窗口',
  },
  'sleepy-beneficiary': {
    prompt: '倒數兩天前就已歸零，收官之筆參與者卻杳無音訊。你用自己的錢包調用收官。會發生什麼？',
    options: {
  ...
  'art-engine': {
    ...
    options: {
      a: '一段對引力三體問題的確定性物理模擬，種子來自鏈上數據。',
      b: '一個在太空攝影上微調過的擴散模型。',
      c: '由藝術家逐幅繪製並上傳。',
      d: '一個套用太空配色的隨機像素生成器。',
    },
    explanation:
      '三個質量相近的天體在牛頓引力下互繞；種子決定初始條件，物理完成其餘一切。任何階段都沒有生成式模型——沒有訓練數據，沒有采樣，也沒有提示詞。讓每幅簽名獨一無二的是混沌，而非隨機。',
  ...
  'same-seed': {
    ...
    options: {
      a: '在任何機器上都逐像素一致的同一幅圖像。',
      b: '相似但帶有少量隨機差異的圖像。',
      c: '不同硬件上會得到不同的圖像。',
      d: '只有低分辨率預覽；完整作品需要項目伺服器。',
    },
    explanation:
      '確定性靠機制保證，而非口頭承諾：同一種子在任何機器上都產出逐比特一致的圖像。生成幀的 SHA-256 哈希在持續集成中逐一斷言，輸出一旦漂移，構建立即失敗。',
    referenceLabel: '白皮書 §6.2 · 可復現性與許可',
  },
  'cst-supply-origin': {
    prompt: 'CST 從哪裏來？',
    options: {
      a: '供應量從零起步，只有協議合約能銘刻它——流通中的每一枚 CST 都能追溯到某個週期裏的一次參與。',
      b: '上線時為團隊預留了一大筆份額。',
      c: '上線前免費發放給了早期錢包。',
      d: '任何人調用代幣合約都能銘刻 CST。',
    },
  ...
  'cst-on-spend': {
    prompt: 'Rio 花了一些 CST 落下一筆。這些 CST 去了哪裏？',
    options: {
      a: '被銷燬——永久移出供應量。',
      b: '進了團隊的金庫。',
      c: '匯入週期儲備，收官時再次發放。',
  ...
  'public-goods-beneficiary': {
    prompt: `每個週期都將儲備的 ${protocolFacts.publicGoodsPercentage}% 作為公共物品分配轉撥出去。目前的受益方是誰？`,
    options: {
    ...
    explanation:
      '公共物品金庫把這一份額轉撥給 Protocol Guild，轉撥作為收官的一部分在鏈上強制執行——沒有人逐週期決定是否兌現。道理很簡單：活在公共基礎設施之上的協議，就該機械地、按節奏地、公開地資助這層設施。',
    referenceLabel: '白皮書 §10 · 公共物品',
  },
  'anchoring-basic': {
    prompt: 'Mira 把她的 Cosmic Signature NFT 錨定至協議。錨定為她帶來什麼？',
    options: {
      a: `錨定期間，這枚 NFT 按比例累積每週期 ${protocolFacts.anchorDistributionPercentage}% 的錨定派發，累積的 ETH 在解錨時取回。`,
      b: '把 NFT 掛到市場上出售。',
      c: '把 NFT 轉換成 CST。',
      d: '用新的種子重新渲染作品。',
    },
    explanation:
      '錨定是協議的長期對齊方式：已錨定的 Cosmic Signature NFT 按比例分攤錨定派發，累積的 ETH 在解錨時取回。NFT 本身絲毫不變——種子與作品是永久的。',
  ...
  'random-walk-perk': {
    ...
    options: {
      a: `這一筆的價格降低 ${protocolFacts.randomWalkDiscountPercentage}%；NFT 留在 Sol 的錢包裏，只被標記為已使用——一生一次。`,
      b: 'NFT 被轉移給協議，以換取降價。',
      c: '這一筆變成免費。',
  ...
  'message-on-gesture': {
    ...
    options: {
      a: `一條最長 ${protocolFacts.gestureMessageMaxLength} 字節、記錄在鏈上的訊息，外加可附加的 ERC-20 代幣或一枚 ERC-721 NFT。`,
      b: '什麼都不能——落筆只是價值轉移。',
      c: '一張儲存在合約裏的圖片。',
      d: '不限長度的文字，儲存在鏈下。',
    },
    explanation: `落筆可攜帶最長 ${protocolFacts.gestureMessageMaxLength} 字節的訊息，與落筆一同記錄在鏈上；也可附加代幣或 NFT。附加資產由分配錢包託管，週期收官後受益方享有優先取回權。`,
  ...
  'stellar-selection-what': {
    prompt: '星選是什麼？',
    options: {
      a: '週期進行中按筆計入的資格記錄，收官時由合約從中選出獲配者。',
      b: '按活躍度排名參與者的榜單。',
      c: '賦予 NFT 作品的稀有度等級。',
      d: '為作品中的星座命名的方案。',
    },
  ...
  'ecosystem-optionality': {
    ...
    options: {
      a: '一切照舊——每項機制都可以直接調用合約完成。',
      b: '什麼也做不了，只能等應用程式恢復。',
      c: '只能取回分配，不能落筆。',
      d: '只能用 CST 落筆，不能用 ETH。',
    },
    explanation:
      '合約之外的生態——應用程式、Axiom Zero、Uniswap 流動性、Chaos Zero——是便利，不是依賴。它們都不是必需的：落筆、收官、錨定與取回，都可以直接調用已驗證的合約完成。',
  ...
  'what-it-is-not': {
    ...
    options: {
      ...
      b: '取得 CST 是一條憑他人努力獲取金錢的可靠途徑。',
      c: '有一個運營方從每個週期中留下一定比例。',
      d: '協議承諾 NFT 的價值會隨時間上漲。',
    },
    explanation:
      '每條分配軌道流向的都是參與者、已錨定的 NFT、滾動儲備或公共物品——不存在運營方抽成。白皮書不對價格、流動性或未來價值作任何承諾，並直言任何人都不應帶着「憑他人努力獲取金錢」的預期取得 CST 或 NFT。',
    referenceLabel: '白皮書 §14.1 · Cosmic Signature 不是什麼',
  },
  'where-recorded': {
    prompt: '落筆、種子與週期歷史究竟存放在哪裏？',
    options: {
      a: '在鏈上——Arbitrum One，一條以太坊 Layer 2 網絡。',
      b: '在項目的私有數據庫裏。',
      c: '只在團隊維護的 IPFS 文件裏。',
      d: '不作記錄，只保留彙總數據。',
    },
    explanation:
      '協議運行在 Arbitrum One 上，重要記錄——每一筆落筆、每一個種子、每一份分配——都在鏈上。正因如此，作品可復現、發放可審計，任何人都無需信任任何伺服器。',
