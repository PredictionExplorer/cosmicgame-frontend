...
export const hardQuestionsTextZhHk = {
  ...
  'owner-mid-cycle': {
    ...
    options: {
      a: '什麼也做不了——週期運行期間核心參數全部鎖定，合約不可升級；所有者的操作只存在於週期之間的縫隙裏。',
      b: '可以立即改比例，但不能改代碼。',
      c: '可以升級合約，但不能改參數。',
      d: '有宇宙議會會籤就都可以。',
    },
    explanation:
      '週期運行期間，所有者不能更改比例、增量或價格，也不能升級合約。協議刻意不設任何在週期進行中修改合約的機制，無論情形如何——參與者行動時所見的規則，就是結算週期的規則。',
    referenceLabel: '白皮書 §13 · 全面去中心化之路',
  },
  'owner-cannot-reach': {
    prompt: '下列哪些是所有者即使在週期之間也能觸碰的？',
    options: {
      a: '都不能：託管中的分配、已銘刻的 NFT、已記錄的種子與 CST 餘額，全部超出一切所有者權限。',
      b: '託管中的分配可以，其餘不行。',
      c: '已記錄的種子可以，用於修復有問題的作品。',
      d: '緊急情況下可以動 CST 餘額。',
    },
    explanation:
      '任何所有者權限都觸不到託管中的分配、已銘刻的 NFT、已記錄的種子或任何人的 CST 餘額——也沒有任何團隊錢包從落筆中收取 ETH。所有者真正的權限很窄：週期之間的時間調整，以及元數據 URI、金庫受益方這類外圍管理。',
    referenceLabel: '白皮書 §13 · 全面去中心化之路',
  },
  'owner-endgame': {
  ...
  'art-integrator': {
    ...
    explanation:
      '辛積分器尊重哈密頓系統的幾何結構，能量在上百萬步中不會漂移——軌道在整段模擬裏保持物理上的誠實。也不存在解析解可以抄近路：三體問題沒有一般解析解，而這正是整件藝術的立足點。',
  ...
  'art-server-death': {
    ...
    explanation:
      '種子全部在鏈上，渲染管線開源且確定，這個系列因此不依賴任何伺服器。任何人都能僅憑鏈上數據逐像素重新生成任何一幅簽名——這是一個 NFT 系列所能給出的最強存續承諾。',
  ...
  'hacken-findings': {
    ...
    explanation:
      '這次審查於2026年1月發佈，覆蓋核心協議、CST 代幣、兩個 NFT 集成、錨定錢包與配套合約。對這種規模的系統，「零發現」本身才是危險信號；真正重要的是嚴重度分佈——無嚴重級、無高危級——以及報告全文公開這一事實。',
  ...
  'verification-tooling': {
    ...
    explanation:
      '這些層層疊加：形式化驗證規範（Certora）、SMT 檢查、靜態分析（Slither）、覆蓋率導向的測試——再加上 Hacken 的審查與模糊測試。沒有哪個工具能證明一切，這正是嚴肅項目全都要跑一遍的原因。',
    referenceLabel: '白皮書 §11.1 · 獨立審查',
  },
  'sourcify-status': {
