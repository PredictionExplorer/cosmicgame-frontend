 ...
 */
export const hardQuestionsTextZhHk = {
  ...
  'refusing-beneficiary': {
    prompt: '一個自動化合約錢包握有收官之筆，卻被寫成拒收一切 ETH。它調用收官。會發生什麼？',
    options: {
  ...
  'refusing-chrono': {
    ...
    explanation:
      '取回優先於送達：次級 ETH 分配存入托管，而非在收官時直接發送，任何獲配方合約都無法藉此阻塞週期落幕。這個惡意錢包的 ETH 在分配錢包裏等着——若長期無人取回，最終會成為任何人都可取回的分配。',
    referenceLabel: '白皮書 §11.2 · 防禦式設計',
  },
  'public-goods-transfer-fails': {
    prompt: '收官過程中，公共物品轉撥無法完成。協議怎麼辦？',
    options: {
    ...
    explanation:
      '容錯轉撥是刻意的設計選擇：公共物品金庫出問題，絕不能讓週期擱淺。收官照常完成，失敗記錄在鏈上，轉撥事後處理。對比受益方的直接送達——那筆轉帳確實會回滾，但只回滾調用者自己的交易。',
    referenceLabel: '白皮書 §11.2 · 防禦式設計',
  },
  'no-anchored-nfts': {
    prompt: '某週期收官時，沒有任何已錨定的 Cosmic Signature NFT。錨定派發怎麼辦？',
    options: {
    ...
    explanation:
      '收官時若沒有任何已錨定的 Cosmic Signature NFT，錨定派發將跳過，這部分份額隨滾動儲備滾入下一週期。不為未來的錨定者留存任何東西——每個週期的發放只看收官那一刻的錨定集合。',
  ...
  'no-cst-gestures': {
    ...
    explanation:
      '沒有 CST 落筆的週期直接跳過 CST 收官之筆軌道——不會另立替補獲配者，收官照常進行。同樣的「跳過而不替補」原則也適用於沒有錨定的週期裏的錨定 NFT 星選。',
    referenceLabel: '白皮書 §5.1 · 收官時的發放',
  },
  'randomness-sources': {
    prompt: '星選與藝術種子背後的隨機性來自哪裏？',
    options: {
      a: '一個鏈上構造：摺疊上一區塊哈希、目前基礎費與來自 Arbitrum 預編譯合約的熵，再以 keccak256 逐個導出隨機值。',
      b: '一份 Chainlink VRF 預言機訂閱。',
      c: '週期參與者之間的提交-揭示儀式。',
      d: '每次收官前由團隊提交的種子。',
    },
    explanation:
      '種子把上一區塊哈希、目前基礎費與來自 ArbSys 和 ArbGasInfo 預編譯合約的 Arbitrum 專屬熵（上一 Arbitrum 區塊哈希、gas 積壓量與 L1 計價計數器）摺疊在一起。這是有意的極簡：不引入預言機，不依賴外部委員會，也沒有任何可能讓週期擱淺的回調。',
  ...
  'precompile-unavailable': {
    ...
    options: {
      a: '預編譯調用具備容錯性；構造退回其餘來源繼續。',
      b: '收官回滾，直到預編譯恢復。',
      c: '週期每小時重試一次，直到成功。',
      d: '由所有者提供替代種子。',
    },
    explanation:
      '每個熵來源在設計上都是可選的：某個預編譯調用不可用時，構造就摺疊其餘來源。這一主題貫穿協議——任何外部事物，哪怕是 Arbitrum 自己的預編譯合約，都不得挾持收官。',
    referenceLabel: '白皮書 §11.3 · 隨機性',
  },
  'v2-flat-cst-problem': {
  ...
  'v2-min-imprint-guard': {
    ...
    explanation:
      '參與 CST 取決於距上一筆的時間。在簽名與交易執行之間，若他人先落筆，這段間隔便會縮短。最低銘刻保護讓參與者事先設定下限：實際銘刻量低於下限時，交易便會還原。',
    referenceLabel: '白皮書 §12.2 · V2 升級',
  },
  ...
  'v3-shape': {
    ...
    options: {
      a: '溢價在窗口的大部分時間裏幾乎無感，只在最後陡然直立——距收官 10 分鐘約 1.04 倍，5 分鐘約 1.9 倍，1 分鐘約 7 倍，到點 10 倍。',
      b: '它讓溢價在整個窗口內線性上升。',
      c: '它讓整個窗口都按 10 倍計價。',
      d: '它隻影響 CST 落筆。',
    },
  ...
  'v3-overtime': {
    ...
    explanation:
      '溢價攀升到 10 倍後便停在那裏，超時落筆一律按 10 倍計。到期後的接任依然可行——V2 的儲存時間規則仍然管轄時鐘——但在 V3 之下，它們是昂貴的表態，而非免費的偷襲。',
    referenceLabel: '白皮書 §12.3 · 規劃中的 V3 升級',
  },
  'owner-mid-cycle': {
    prompt: '週期進行中，所有者想改一個比例、再升級合約。此刻所有者究竟能做什麼？',
    options: {
      a: '比例調整與升級都必須等到週期間進行；週期運作期間，核心參數保持鎖定。',
      b: '可以立即改比例，但不能改代碼。',
      c: '可以升級合約，但不能改參數。',
      d: '有宇宙議會會籤就都可以。',
    },
    explanation:
      '週期運作期間，所有者不能更改核心分配比例、時間增量或落筆價格，也不能升級協議合約。部分外圍設定等較窄權限，則有各自的適用條件。',
    referenceLabel: '白皮書 §13 · 全面去中心化之路',
  },
  'owner-cannot-reach': {
    prompt: '下列哪些是所有者即使在週期之間也能觸碰的？',
    options: {
      a: '文檔所列的管理函數都不能直接改動這些資產的歸屬或重寫這些記錄。',
      b: '託管中的分配可以，其餘不行。',
      c: '已記錄的種子可以，用於修復有問題的作品。',
      d: '緊急情況下可以動 CST 餘額。',
    },
    explanation:
      '文檔所列的管理函數不會直接改動託管分配、已銘刻 NFT 的歸屬、已記錄的種子或 CST 餘額。在交出特權控制前，所有者仍保有另行披露的週期間升級權限。',
    referenceLabel: '白皮書 §13 · 全面去中心化之路',
  },
  'owner-endgame': {
    ...
    explanation:
      '承諾是明確的：以 V3 為首的剩餘升級完成後，所有者角色將永久離開部署者，具體方式提前公佈。自那時起，任何私人主體都無法再升級協議或更改參數——而這一進程的每一步都在鏈上公開可見，包括最後一步。',
    referenceLabel: '白皮書 §13 · 全面去中心化之路',
  },
  'postpone-activation-limit': {
    prompt: '所有者想把下一週期的啟用時間向後推。這項權限的邊界在哪裏？',
    options: {
    ...
    explanation:
      '推遲下一週期啟用是隨時可用的三項窄權限之一——但僅限其首筆落筆到來之前。有人落筆的那一刻，週期便已啟程，所有者手裏的時間槓桿隨之消失。',
    referenceLabel: '白皮書 §13 · 全面去中心化之路',
  },
  'no-team-eth': {
    ...
    explanation: `推廣儲備每週期接收 ${cst(protocolFacts.outreachReserveCst)} CST 用於社區推廣——這是團隊經手的唯一一條固定流，且不附帶任何特殊權限。至於 ETH，白皮書說得斬釘截鐵：沒有團隊錢包從落筆中收取 ETH。`,
  ...
  'art-integrator': {
    ...
    explanation:
      '辛積分器保持哈密頓系統的幾何結構，適合維持長時間模擬中的能量穩定性，但不保證每一步都精確滿足能量守恆。三體問題沒有一般解析解，因此管線採用數值積分。',
  ...
  'art-candidates': {
    ...
    explanation:
      '種子生成十萬組候選構型；每組推進一百萬個物理步；Borda 排序聚合按混沌與等邊兩項指標選出最出挑的一條。每個階段都是種子的純函數——遴選由算法完成，任何人都能一模一樣地復現。',
    referenceLabel: '白皮書 §6.1 · 渲染管線',
  },
  'art-color': {
    prompt: '三個天體的色彩如何確定？',
    options: {
      a: '在 OKLab 感知色彩空間中混合，各天體色相相隔 120°，並由漂移與正弦波調製。',
      b: '每幅簽名固定使用紅、綠、藍三色。',
      c: '從真實星雲照片中取樣。',
      d: '銘刻時由 NFT 的第一位所有者挑選。',
    },
    explanation:
      '色彩在 OKLab 中混合——在這個感知色彩空間裏，數值上等距的顏色在人眼看來差異也相等——各天體色相相隔 120°，彼此始終分明。與種子之後的一切一樣，配色是確定性的。',
    referenceLabel: '白皮書 §6.1 · 渲染管線',
  },
  'art-spectral': {
    prompt: '軌跡為何呈現出那樣的質感？',
    options: {
      a: '光譜渲染：從 380 至 700 納米劃分 64 個波長區間，以隨速度變化的線寬和景深渲染軌跡。',
      b: '加了輝光濾鏡的扁平矢量筆畫。',
      c: '物理模擬器的截圖，再交給 AI 放大。',
      d: '在模擬之上手工調校的樣條曲線。',
    },
    explanation:
      '渲染器把光當作光譜，而非三條顏色通道：可見光範圍內的 64 個波長區間、隨速度變化的線寬、營造縱深的景深。AgX 色調映射、輝光、OpenSimplex 星雲層與色彩分級共同完成畫面。',
    referenceLabel: '白皮書 §6.1 · 渲染管線',
  },
  'art-output': {
    prompt: '管線為每枚 NFT 輸出什麼文件？',
    options: {
      a: '一張 16 位 PNG 與一段 30 秒 H.265 影片。',
      b: '只有一張 JPEG 縮略圖。',
      c: '一張動態 GIF。',
      d: '一個 SVG 矢量文件。',
    },
    explanation:
      '每幅簽名交付一張 16 位 PNG——每通道色深是常規的兩倍——外加一段 30 秒的 H.265 軌道運動影片。兩者都可以由任何人用鏈上種子跑一遍開源管線重新生成。',
    referenceLabel: '白皮書 §6.1 · 渲染管線',
  },
  'art-server-death': {
    prompt: '與項目相關的所有伺服器明天全部消失。作品會怎樣？',
    options: {
    ...
    explanation:
      'NFT 種子保存在鏈上，渲染管線開源且具有確定性。任何人只要擁有種子和管線，就能重新生成作品，無需依賴項目的圖像伺服器。',
    funFact: '持續集成逐幀斷言渲染結果的 SHA-256 哈希——管線哪怕意外漂移一個像素，構建都會失敗。',
    referenceLabel: '白皮書 §6.2 · 可復現性與許可',
  },
  'art-naming': {
    prompt: '所有者對自己的 Cosmic Signature NFT 有哪些自定義空間？',
    options: {
      a: '可在鏈上為它命名，最長 32 字節——作品本身永不改變。',
      b: '可以重擲一次種子。',
      c: '可以調整配色。',
      d: '可以延長影片時長。',
    },
    explanation:
      '所有者可在鏈上記錄一個最長 32 字節的名字。自定義空間到此為止：種子、軌道、配色與影片在銘刻那一刻永久固定——確定性是這個系列的核心承諾，重擲種子會將其擊碎。',
    referenceLabel: '白皮書 §6.2 · 可復現性與許可',
  },
  'art-license': {
    prompt: '項目自有的合約、着色器與渲染管線以什麼許可發佈？',
    options: {
    ...
    explanation:
      '項目自有代碼均以 CC0 1.0 獻入公有領域，不保留任何權利：任何人都可以復刻合約、渲染器或網站。第三方依賴保留各自的許可。就連白皮書本身也是 CC0。',
    referenceLabel: '白皮書 §6.2 · 可復現性與許可',
  },
  'seed-derivation': {
    prompt: 'NFT 的藝術種子在何時、如何產生？',
    options: {
      a: '銘刻時由合約從鏈上數據導出 32 字節種子，與 NFT 一同儲存；一個 SHA3-256 生成器讓此後的一切都是它的純函數。',
      b: '收官前由藝術家為每枚 NFT 上傳種子。',
      c: '種子就是所有者的錢包地址。',
      d: '每次渲染都會重新取一個種子。',
    },
    explanation:
      '種子在銘刻時於鏈上導出，並與 NFT 永久儲存。它初始化一個 SHA3-256 隨機數生成器，此後的每個選擇——候選軌道、鏡頭、色彩——都是它的純函數。今天渲染，或十年後渲染：同一種子，同一幅簽名。',
    referenceLabel: '白皮書 §6.1 · 渲染管線',
  },
  'hacken-findings': {
    ...
    explanation:
      '最終報告於2026年1月發佈，涵蓋核心協議、CST 代幣、兩個 NFT 集成、錨定錢包與配套合約。閱讀時應一併查看問題嚴重度、處理狀態和審查範圍。報告沒有嚴重或高危級別發現，但這不等於協議不存在風險。',
    referenceLabel: '白皮書 §11.1 · 獨立審查',
  },
  'hacken-invariants': {
    prompt: '人工審查之外，Hacken 的模糊測試檢驗了什麼？',
    options: {
      a: '14 項系統不變量——例如協議持有的 ETH 總額必須始終等於存入減去發放——全部在 10,000 次運行中保持成立。',
      b: '常見交易的 gas 消耗。',
      c: '藝術管線的確定性。',
      d: '前端渲染性能。',
    },
    explanation:
      '模糊測試用生成的輸入反覆衝擊系統，同時斷言必須永遠成立的性質。Hacken 形式化了 14 項這樣的不變量，全部在 10,000 次運行中保持成立——這與逐行審查是兩種不同的證據，專門瞄準沒有人想到要試的狀態。',
    referenceLabel: '白皮書 §11.1 · 獨立審查',
  },
  'verification-tooling': {
    prompt: '外部審查之外，代碼儲存庫還帶有哪些驗證工具？',
    options: {
    ...
    explanation:
      'Certora 規範、SMTChecker、Slither 與覆蓋率導向的測試，分別檢查不同性質與缺陷類型。它們與 Hacken 審查和模糊測試互為補充；任何單項檢查都不能證明系統完全正確。',
    referenceLabel: '白皮書 §11.1 · 獨立審查',
  },
  'sourcify-status': {
    prompt: '已部署合約的源碼驗證狀態如何？',
    options: {
    ...
    explanation:
      '全部合約都在 Sourcify 上以精確匹配狀態完成驗證（鏈 ID 42161）——這是最嚴格的驗證等級：鏈上字節碼與公開源碼逐字節一致，連元數據也不例外。代理地址是協議的永久地址，實現只會經由公開升級流程更替。',
  ...
  reentrancy: {
    ...
    explanation:
      '重入防護會阻止受保護入口在同一筆尚未完成的交易中被再次調用。次級分配採用託管取回方式，也避免在收官期間直接向獲配者轉帳。這些防護針對特定風險，不代表所有攻擊都已被排除。',
    referenceLabel: '白皮書 §11.2 · 防禦式設計',
  },
  'intercycle-locks-why': {
    prompt: '為什麼週期運行期間合約不可升級——哪怕出了緊急情況？',
    options: {
  ...
  'rwlk-not-transferred': {
    prompt: 'Sol 為降價附加了他的 Random Walk NFT。之後這枚 NFT 在哪裏？',
    options: {
      a: '仍在他的錢包裏——合約只是將其標記為已使用；它從未被轉移或託管。',
      b: '託管在分配錢包裏，直到週期結束。',
      c: '被銷燬，換取降價。',
      d: '轉移給協議，收官後歸還。',
    },
    explanation:
      'Random Walk NFT 從不移動：合約記下「已使用」，並套用價格減免。被消耗的是這個標記——每枚一生一次，橫跨所有週期——一個數量固定的外部系列由此融入協議經濟，而協議不必保管任何東西。',
  ...
  'eth-window-duration-drift': {
    ...
    options: {
      a: '按上線參數約兩天——但時長與週期時間增量掛鈎，會隨協議年歲緩慢拉長。',
      b: `永遠固定 ${protocolFacts.finalGestureExclusivityHours} 小時。`,
      c: `固定 ${protocolFacts.initialCstCalibrationWindowHours} 小時，與 CST 窗口的基準一致。`,
      d: '活躍度越高，每個週期越短。',
    },
    explanation: `按上線參數，整段下行約需兩天；若窗口走完仍無人落筆，價格便停在底價等待。時長與時間增量掛鈎——增量每週期增長 ${protocolFacts.cycleTimeIncrementIncreasePercentPerCycle}%——窗口因此隨協議逐漸舒展的節拍一同拉長，而非一成不變。`,
  ...
  'recognition-vs-participation': {
    ...
    explanation: `三條途徑是：參與 CST（落筆時按公式銘刻）、表彰 CST（收官時隨每份 NFT 發放 ${cst(protocolFacts.specialAllocationCst)} CST），以及推廣儲備（每週期 ${cst(protocolFacts.outreachReserveCst)} CST）。只有最後一條由團隊經手——而它只是普通 CST，不附帶任何特殊權限，用於社區推廣。`,
    referenceLabel: '白皮書 §7.1 · 銘刻規則',
  },
  'finalization-actions': {
    prompt: '收官是一筆交易。它究竟做了什麼？',
    options: {
      a: '讀取一次協議的 ETH 餘額，發放各分配軌道，銘刻本週期的 NFT 與 CST，為每件作品記錄種子，並排定下一週期。',
      b: '只轉出受益方的 ETH；其餘事後處理。',
      c: '只銘刻 NFT；ETH 發放分成多筆交易陸續完成。',
      d: '啟動一個由團隊經手、歷時數日的結算流程。',
    },
    explanation:
      '一筆交易結清整個週期：讀取一次 ETH 餘額，發放第 5 節的各條軌道（受益方與公共物品直接送達，其餘託管），銘刻 NFT 與 CST 並記錄種子，排定下一週期。原子性正是要點——不存在「收官到一半」的狀態。',
    referenceLabel: '白皮書 §3.3 · 收官與公開收官窗口',
  },
  ...
  'anchored-rwlk-weighting': {
    ...
    explanation: `錨定 NFT 星選每週期在已錨定的 Random Walk NFT 中選出 ${protocolFacts.anchoredRwlkNftSelectionRecipients} 次，權重按各錨定者錨定的數量計。每次攜帶 ${cst(protocolFacts.specialAllocationCst)} CST 與一枚 Cosmic Signature NFT——不含 ETH，ETH 派發專屬於已錨定的 Cosmic Signature NFT。`,
    referenceLabel: '白皮書 §5.3 · 星選',
  },
  'voluntary-vault-contributions': {
    prompt: '除了每週期的轉撥，ETH 還能透過別的途徑進入公共物品金庫嗎？',
    options: {
    ...
    referenceLabel: '白皮書 §10 · 公共物品',
  },
  'risk-honesty': {
    prompt: '下列哪一條是白皮書自己的風險章節所承認的？',
    options: {
      a: '審查與形式化分析都不構成保證——任何持有價值的軟件都可能存在未知缺陷。',
      b: '合約已被數學證明不含任何缺陷。',
      c: '風險只存在到 V3 升級上線為止。',
      d: '唯一真正的風險是以太坊本身失敗。',
    },
    explanation:
      '風險因素毫不粉飾：智能合約風險在每一輪審查之後依然存在；隨機性有明說的邊界；期限是真實的截止時間；去中心化完成之前，參數仍可能在週期間調整；資產價值會波動。應把落筆當作為參與和藝術付出的花費——這是白皮書自己的措辭。',
