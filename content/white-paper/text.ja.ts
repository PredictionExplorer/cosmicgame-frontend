  ...
  sections: {
    introduction: {
      ...
      blocks: [
        {
          kind: 'paragraph',
          text: 'Cosmic Signatureは二つの確信から始まりました。一つは、ジェネラティブアートが最も面白くなるのは、構成の一つひとつに根拠があるとき——すべての画像が物理的な過程の出力であり、シードによって固定され、誰でもその過程を再実行して結果を検証できるとき——だということです。もう一つは、参加者に代わってETHを保持するプロトコルは、すべてのweiがどこへ行くのかという問いに、ルールに基づく明確な答えを示す責任があるということです。',
        },
        {
          kind: 'paragraph',
          text: 'その結果が、時間を軸に築かれたプロトコルです。パフォーマンス・サイクルが開き、一筆で満たされ、確定時刻を過ぎると確定できます。一筆は小さなオンチェーンの行為です。ETHまたはCSTを携え、短いメッセージや添付されたアセットを携えることもあり、サイクルの確定時刻をさらに未来へ押し出します。確定時刻を過ぎると、最後に一筆を入れた参加者がサイクルを確定できます。確定は準備金を配り、サイクルのNFTを刻印し、次のサイクルを準備します。',
        },
        {
          kind: 'paragraph',
        ...
        },
        {
          ...
          items: [
            ...
            '機械的な分配。配分ルールは検証済みのコントラクトに記述され、確定時に実行されます。参加者と分配のルールの間に裁量を持つアカウントは存在せず、一筆からETHを受け取るチームのウォレットもありません。',
    ...
    'protocol-overview': {
      ...
      blocks: [
        {
          kind: 'paragraph',
          text: 'システムは一つの中核コントラクトと、特定の役割を担う複数の補助コントラクトで構成されます。アップグレード可能なプロキシの背後にデプロイされた中核がサイクルを動かします。一筆の費用を決め、カウントダウンを追い、サイクル準備金を保持し、確定を実行します。その周りに、CSTトークン、Cosmic Signature NFTコレクション、配分のためのエスクローウォレット、二つの係留ウォレット、公共財金庫、広報準備金、そして宇宙評議会があります。',
        },
        {
          kind: 'table',
          table: {
    ...
    'performance-cycle': {
      ...
      blocks: [
        {
          kind: 'paragraph',
          text: 'サイクルは、一筆の受付から確定までの期間です。費用が下がっていく調律期間で開き、一筆で満たされ、サイクル確定時刻を過ぎて誰かがそれを確定すると終わります。本節は時計を扱い、第4節は一筆そのものを扱います。',
        },
      ],
      subsections: {
        ...
        finalization: {
          ...
          blocks: [
            {
              kind: 'paragraph',
              text: 'サイクル確定時刻を過ぎると、最後の一筆を入れた参加者が確定できるようになります。確定は一つのトランザクションです。プロトコルのETH残高を読み取り、第5節の配分トラックを配り、サイクルのNFTとCSTを刻印し、各新しいアートワークのシードを記録し、次のサイクルを予定します。',
            },
            {
              kind: 'paragraph',
              text: `最後の一筆の参加者はこの権利を${protocolFacts.finalGestureExclusivityHours}時間、独占的に保ちます。その後、公開確定期間が始まります。誰でも確定でき、コントラクトは確定した人を、その役割が携えるすべて——シグネチャー配分のETHの分、そのCSTの刻印、そのNFT、添付されたアセットへの優先権——とともにサイクルの受領者として扱います。このルールは、最後の参加者が手続きしなくてもプロトコルを進められるようにするものです。優先期間を過ぎると、最初に確定した人がシグネチャー配分の受領者になります。`,
            },
            {
              kind: 'paragraph',
    ...
    gestures: {
      ...
      blocks: [
        {
          kind: 'paragraph',
          text: '一筆はサイクルを進める基本的な参加行為です。通貨にかかわらず、それぞれがカウントダウンを延ばし、サイクルの参加者の星選に1件の対象を記録し、第5.2節の持久の時計を更新し、第7.1節に述べるとおり参加CSTを刻印することがあります。',
        },
      ],
      subsections: {
        'eth-gestures': {
        ...
        'cst-gestures': {
          ...
          blocks: [
            ...
            {
              kind: 'paragraph',
              text: `期間の長さも参加状況に応じて変わるパラメーターで、ETHとCSTの利用を調整する仕組みの一つです。それは${protocolFacts.initialCstCalibrationWindowHours}時間の基準から始まりました。ETH一筆はそれぞれ約${protocolFacts.cstCalibrationWindowDecreasePercentPerEthGesture}%短くし、CST一筆はそれぞれ約${protocolFacts.cstCalibrationWindowIncreasePercentPerCstGesture}%長くします。したがって、ETHの活動が盛んならCSTの下降は速まり、CST一筆はより早く魅力的になります。CSTの活動が盛んならまた遅くなります。この仕組みにより、ETHとCSTの利用状況が互いの費用に反映されます。`,
            },
            {
              kind: 'paragraph',
      ...
      subsections: {
        'distribution-at-finalization': {
          ...
          blocks: [
            ...
            {
              kind: 'paragraph',
              text: '配分される五つのETHトラックの合計は準備金の半分です。残りは次のサイクルへ持ち越されます。準備金が毎回増えるとは限らず、その規模は参加状況と配分によって変わります。Cosmic Signature NFTが一つも係留されていない状態でサイクルが確定すれば、そのサイクルの係留配分は飛ばされ、その分も累積します。',
            },
            {
              kind: 'table',
              table: {
        ...
        'endurance-and-chrono': {
          ...
          blocks: [
            {
              kind: 'paragraph',
              text: '二つのトラックは、異なる保持時間を測ります。持久チャンピオンは、一筆を入れてから次の一筆が入るまで、最も長く先頭を保持した参加者です。時の戦士は、その持久チャンピオンの称号を最も長く連続して保持した参加者です。',
            },
            {
              kind: 'paragraph',
              text: '例を見ると、二つの違いがわかります。静かな午後に一筆を入れ、十時間ほかの一筆が入らなかった参加者は、長い先頭保持時間を記録します。その参加者がサイクルをその時の戦士として終えるかどうかは、別の参加者がそれを上回るまでにその記録がどれだけ長く保持されるかによります。持久は自分が作った間隔を測り、時の戦士のトラックは自分の記録がどれだけ長く持ちこたえたかを測ります。どちらも確定時にだけ決着します。',
            },
          ],
        },
        'stellar-selections': {
        ...
        'delivery-and-timeouts': {
          ...
          blocks: [
            ...
            {
              kind: 'paragraph',
              text: `エスクローされた配分と添付されたアセットは${protocolFacts.secondaryRetrievalTimeoutWeeks}週間待ちます。その後、コントラクトは未受け取りの配分を誰でも自分のものとして受け取ることを許します。このルールは公開確定期間と対になっています。プロトコルの中に不在の参加者を永遠に待つものはなく、期限後は別の参加者が受け取れるようになります。早めに受け取ってください。`,
            },
          ],
        },
      },
    ...
    cst: {
      ...
      blocks: [
        {
          kind: 'paragraph',
          text: 'CSTはプロトコルのERC-20トークンです。供給量はゼロから始まり、トークンコントラクトは刻印と焼却の指示をプロトコルのコントラクトからだけ受け付けます。流通しているCSTはすべて、参加CST、功労CST、広報準備金など、プロトコルで定めた仕組みによって刻印されます。',
        },
      ],
      subsections: {
        'imprint-rules': {
        ...
        'supply-dynamics': {
          ...
          blocks: [
            {
              kind: 'paragraph',
              text: `CSTは使われるたびに流通から離れます。すべてのCST一筆の費用の全額が焼却されます。したがって供給量は振る舞いによって形づくられます。静かなサイクルはわずかな参加CSTしか刻印せず、活発なCSTの使用は焼却によって供給量を減らし、固定の功労と広報の流れは標準的なサイクルごとに予測できる${cst(protocolFacts.typicalCstImprintsPerCycle)} CSTを加えます。供給上限や事前刻印はありません。広報準備金への継続的なCST配分は、第7.1節のとおりです。`,
            },
            {
              kind: 'paragraph',
              text: '平方根の式はそれ自体が供給の制御で、V2アップグレード（第12.2節）で導入されました。元の設計は一筆あたり一律100 CSTを刻印し、機械の速さの一筆の連続を新しいCSTの無限の源にしてしまいました。現在のルールのもとでは、ごく短い間隔の一筆では参加CSTがほぼゼロとなり、間隔が空くほど増えます。この式は参加CSTに適用され、功労CSTと広報準備金には別の刻印ルールがあります。',
            },
          ],
        },
        'coordination-weight': {
    ...
    anchoring: {
      ...
      blocks: [
        ...
        {
          kind: 'paragraph',
          text: '「一度だけ」のルールは、通常のロックのスケジュールを一回限りの取り消せない選択に置き換え、係留された集合に本物の退出コストを与えます。係留を続けるかはサイクルごとに判断できますが、一度解除すると同じNFTを再び係留することはできません。',
        },
        {
          kind: 'paragraph',
          text: `Random Walk NFTの係留には、別の役割があります。係留中のRandom Walk NFTは係留NFT星選（第5.3節）で選定を受け、サイクルごとに${protocolFacts.anchoredRwlkNftSelectionRecipients}回、それぞれが${cst(protocolFacts.specialAllocationCst)} CSTとCosmic Signature NFTを携えます。Random Walkの係留はETHは配分されません。同じ「一度だけ」のルールが当てはまります。`,
        },
      ],
    },
    'cosmic-council': {
    ...
    'public-goods': {
      ...
      blocks: [
        {
          kind: 'paragraph',
          text: `すべてのサイクルはサイクル準備金の${protocolFacts.publicGoodsPercentage}%を公共財金庫へ送ります。その受け手は現在Protocol Guild——170人以上のEthereumコアプロトコル貢献者の共同資金支援メカニズム——です。送付は確定の一部としてオンチェーンで強制されます。それを守るかどうかをサイクルごとに誰かが決めることはありません。プロトコルの利用が増えるほど、Ethereumを支えるインフラへ届く資金も増えます。`,
        },
        {
          kind: 'paragraph',
        ...
        },
        {
          ...
          kind: 'note',
          text: 'これは公共財のアドレス（現在はProtocol Guild）へのETHの送付です。米国の税法上の慈善寄付には当たらず、Cosmic Signatureはいかなる法域においてもその税務上の扱いについて何も表明しません。',
          // lexicon-allow-end
        },
      ],
    },
        ...
        v2: {
          ...
          blocks: [
            ...
            },
            {
              kind: 'list',
              items: [
                '動的な参加CST。一筆あたり一律100 CSTは、第7.1節の平方根の式になりました。一律の刻印は連続する一筆を無償のCSTに変えていました。新しいルールでは、一筆の頻度ではなく、前の一筆からの経過時間に応じて刻印量が決まります。',
                '最小刻印の保護。すべての一筆の方法に、参加者が受け入れる最小の参加CSTの量のパラメーターが加わり、署名と実行の間のタイミングのずれから参加者を守ります。',
                '変動するCST調律期間。期間の長さは一筆の混合に反応する保存された値になり（第4.3節）、ETHとCSTの道筋が互いにバランスを保ちます。',
    ...
    decentralization: {
      ...
      blocks: [
        {
          kind: 'paragraph',
          text: 'プロトコルには現在、所有者がいます。それをデプロイしたアドレスです。この役割には実際の管理権限があります。権限の範囲は限定されており、将来はデプロイ者から取り除く方針です。',
        },
        {
          kind: 'paragraph',
    ...
    conclusion: {
      ...
      blocks: [
        {
          kind: 'paragraph',
          text: 'Cosmic Signatureは、誰の許可も、そしていずれは特定の管理者にも依存しないジェネラティブアート・プロトコルを築く試みです。仕組みは完全に規定できるほど小さいものです。費用が下がっていく期間、一筆が延ばすカウントダウン、コントラクトで定めた配分ルール、累積する準備金、そして物理とシードの純粋な関数であるアート。残るアップグレードはわずかで、公開されています。それが終われば所有者の役割は消え、残るのは本稿が述べるものです。時計、準備金、トークン、評議会、そして途切れることのないシグネチャーの連なり——その一つひとつが、それを形づくった手の記録です。',
        },
      ],
    },
    'appendix-a': {
