  ...
  sections: {
    introduction: {
      ...
      blocks: [
        ...
        {
          kind: 'paragraph',
          text: '그 결과가 시간을 중심으로 설계된 프로토콜입니다. 퍼포먼스 사이클이 시작되고, 제스처로 채워지며, 마감 시각이 지나면 마감할 수 있습니다. 제스처는 작은 온체인 행위입니다. ETH 또는 CST를 담고, 짧은 메시지나 첨부 자산을 실을 수 있으며, 사이클의 마감 시각을 더 뒤로 밀어냅니다. 마감 시각이 지나면 최종 제스처를 남긴 참여자가 사이클을 마감할 수 있습니다. 마감은 준비금을 배분하고, 사이클의 NFT를 각인하며, 다음 사이클을 준비합니다.',
        },
        {
          kind: 'paragraph',
        ...
        },
        {
          ...
          items: [
            ...
            '기계적 배분. 배분 규칙은 검증된 컨트랙트에 정해져 있으며, 마감 시 실행됩니다. 참여자와 배분 규칙 사이에 재량을 행사하는 계정은 없으며, 어떤 팀 지갑도 제스처에서 나온 ETH를 받지 않습니다.',
    ...
    gestures: {
      ...
      blocks: [
        {
          kind: 'paragraph',
          text: '제스처는 사이클을 진행시키는 기본 참여 행위입니다. 통화가 무엇이든 제스처 하나하나는 카운트다운을 연장하고, 사이클의 참여자 별빛 선정에 자격 한 건을 기록하며, 5.2절의 선두 유지 기록을 갱신하고, 7.1절에 설명된 대로 참여 CST를 각인할 수 있습니다.',
        },
      ],
      subsections: {
        'eth-gestures': {
        ...
        'cst-gestures': {
          ...
          blocks: [
            ...
            {
              kind: 'paragraph',
              text: `구간 길이도 참여 상황에 따라 달라지며, ETH와 CST 사용을 조정하는 요소로 작동합니다. 처음에는 ${protocolFacts.initialCstCalibrationWindowHours}시간을 기준으로 시작했습니다. ETH 제스처 하나는 이를 약 ${protocolFacts.cstCalibrationWindowDecreasePercentPerEthGesture}% 줄이고, CST 제스처 하나는 약 ${protocolFacts.cstCalibrationWindowIncreasePercentPerCstGesture}% 늘립니다. 따라서 ETH 활동이 활발하면 CST 하강이 빨라져 CST 제스처가 더 일찍 매력적인 선택이 되고, CST 활동이 활발하면 하강은 다시 느려집니다. 이 구조를 통해 ETH와 CST 사용량의 변화가 서로의 비용에 반영됩니다.`,
            },
            {
              kind: 'paragraph',
      ...
      subsections: {
        'distribution-at-finalization': {
          ...
          blocks: [
            ...
            {
              kind: 'paragraph',
              text: '배분되는 다섯 ETH 경로의 합은 준비금의 절반입니다. 나머지는 다음 사이클로 이월됩니다. 준비금이 매번 커지는 것은 아니며, 규모는 참여 상황과 배분에 따라 달라집니다. 앵커링된 Cosmic Signature NFT가 하나도 없는 상태로 사이클이 마감되면 그 사이클의 앵커링 지급은 건너뛰고 해당 몫도 함께 누적됩니다.',
            },
            {
              kind: 'table',
              table: {
        ...
        'endurance-and-chrono': {
          ...
          blocks: [
            ...
            {
              kind: 'paragraph',
              text: '예를 보면 두 기록의 차이를 알 수 있습니다. 한적한 오후에 제스처를 남기고 열 시간 동안 자리를 내주지 않은 참여자는 강력한 선두 유지 구간을 기록합니다. 그 참여자가 사이클을 시간의 전사로 마치는지는 다른 참여자가 그 기록을 넘어서기 전까지 기록이 얼마나 오래 유지되는지에 달려 있습니다. 선두 유지는 만들어 낸 공백의 길이를 재고, 시간의 전사 경로는 그 기록이 얼마나 오래 유지되었는지를 측정합니다. 둘 다 마감 시점에야 확정됩니다.',
            },
          ],
        },
        'stellar-selections': {
      ...
      subsections: {
        ...
        'reproducibility-and-license': {
          ...
          blocks: [
            {
              kind: 'paragraph',
              text: '결과의 재현성은 자동 검증으로 확인합니다. 같은 시드는 어떤 기기에서도 픽셀 단위까지 같은 이미지를 만들며, 생성된 프레임의 SHA-256 해시는 지속적 통합 과정에서 검증됩니다. 모든 시드가 온체인에 저장되고 파이프라인이 공개되어 있으므로 컬렉션은 어떤 서버에도 의존하지 않습니다. 내일 모든 서버가 사라져도 모든 시그니처를 체인에서 다시 생성할 수 있습니다.',
            },
            {
              kind: 'paragraph',
    ...
    cst: {
      ...
      blocks: [
        {
          kind: 'paragraph',
          text: 'CST는 프로토콜의 ERC-20 토큰입니다. 공급량은 0에서 시작하며, 토큰 컨트랙트는 프로토콜 컨트랙트가 보내는 각인과 소각 지시만 받아들입니다. 유통 중인 모든 CST는 참여 CST, 공로 CST, 홍보 준비금 배분 등 프로토콜에 정해진 규칙에 따라 각인됩니다.',
        },
      ],
      subsections: {
        'imprint-rules': {
        ...
        'supply-dynamics': {
          ...
          blocks: [
            {
              kind: 'paragraph',
              text: `CST는 사용될 때마다 유통에서 빠져나갑니다. 모든 CST 제스처의 비용 전액이 소각됩니다. 따라서 공급량은 행동이 결정합니다. 조용한 사이클은 참여 CST를 적게 각인하고, 활발한 CST 사용은 공급량을 다시 태워 줄이며, 고정된 공로 CST와 홍보 흐름은 일반적인 사이클마다 예측 가능한 ${cst(protocolFacts.typicalCstImprintsPerCycle)} CST를 더합니다. 공급 상한과 사전 각인은 없습니다. 팀이 관리하는 홍보 준비금에는 7.1절에 설명한 CST가 사이클마다 배분됩니다.`,
            },
            {
              kind: 'paragraph',
              text: '제곱근 공식 자체가 V2 업그레이드(12.2절)에서 도입된 공급 조절 장치입니다. 원래 설계는 제스처마다 고정 100 CST를 각인했는데, 이 때문에 자동화된 초고속 제스처가 새 CST의 무한한 원천이 되었습니다. 현재 규칙에서는 빠르게 연달아 남긴 제스처의 참여 CST는 거의 0이며, 이전 제스처와의 간격이 길수록 더 많이 각인됩니다. 이 공식은 참여 CST에만 적용되고, 공로 CST와 홍보 준비금에는 별도의 각인 규칙이 적용됩니다.',
            },
          ],
        },
        'coordination-weight': {
    ...
    anchoring: {
      ...
      blocks: [
        ...
        {
          kind: 'paragraph',
          text: '이 한 번뿐인 규칙은 통상적인 잠금 기간 일정을 단 한 번의 되돌릴 수 없는 선택으로 대체하며, 앵커링된 집합에 실질적인 이탈 비용을 부여합니다. 앵커링을 유지할지는 사이클마다 판단할 수 있지만, 한 번 해제한 NFT는 다시 앵커링할 수 없습니다.',
        },
        {
          kind: 'paragraph',
    ...
    'public-goods': {
      ...
      blocks: [
        {
          kind: 'paragraph',
          text: `모든 사이클은 사이클 준비금의 ${protocolFacts.publicGoodsPercentage}%를 공공재 금고로 전달합니다. 금고의 현재 수령처는 이더리움 핵심 프로토콜 기여자 170여 명을 지원하는 공동 자금 지원 메커니즘인 Protocol Guild입니다. 이 전달은 마감의 일부로 온체인에서 강제되며, 사이클마다 이를 지킬지 말지 누군가가 결정하는 일은 없습니다. 프로토콜이 많이 쓰일수록 이더리움을 지탱하는 인프라로 더 많은 자금이 전달됩니다.`,
        },
        {
          kind: 'paragraph',
        ...
        v2: {
          ...
          blocks: [
            ...
            },
            {
              kind: 'list',
              items: [
                '동적 참여 CST. 제스처마다 고정 100 CST를 각인하던 방식이 7.1절의 제곱근 공식으로 바뀌었습니다. 고정 각인 아래에서는 빠른 제스처 연쇄가 사실상 비용 없는 CST 각인으로 이어졌습니다. 새 규칙은 제스처 빈도가 아니라 이전 제스처 이후의 경과 시간에 따라 각인량을 정합니다.',
                '최소 각인 보호. 모든 제스처 메서드에 참여자가 수용할 최소 참여 CST 양을 지정하는 매개변수가 추가되어, 서명과 실행 사이의 시점 변동에서 참여자를 보호합니다.',
                '참여에 따라 달라지는 CST 보정 구간. 구간의 지속 시간이 제스처 구성에 반응하는 저장값이 되어(4.3절), ETH 경로와 CST 경로가 서로 균형을 유지합니다.',
    ...
    conclusion: {
      ...
      blocks: [
        {
          kind: 'paragraph',
          text: 'Cosmic Signature는 누구의 허락도, 그리고 언젠가는 누구의 관리도 필요하지 않은 제너러티브 아트 프로토콜을 만들려는 시도입니다. 메커니즘은 완전히 명세할 수 있을 만큼 작습니다. 비용이 내려가는 구간, 제스처가 연장하는 카운트다운, 컨트랙트에 정해진 배분 규칙, 이월되는 준비금, 그리고 물리와 시드의 순수 함수인 아트가 전부입니다. 남은 업그레이드는 적고 공개적입니다. 업그레이드가 끝나면 소유자 역할은 사라지고, 이 백서가 설명한 것만 남습니다. 시계 하나, 준비금 하나, 토큰 하나, 평의회 하나, 그리고 끊기지 않고 이어지는 시그니처의 연속. 그 하나하나가 자신을 빚어낸 손의 기록입니다.',
        },
      ],
    },
    'appendix-a': {
