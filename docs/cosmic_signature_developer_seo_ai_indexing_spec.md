...
- **Global 404 with route groups:** every URL no route matches renders [`app/global-not-found.tsx`](../app/global-not-found.tsx) (`experimental.globalNotFound`), a whole document rendered on the server with the host's own chrome: the app header and footer on the app host, the landing header and footer (no wallet stack) on the landing host. The HTML carries the copy, the recovery links and the stylesheet, with a 404 status and `noindex, follow`. A route segment's own `notFound()` (an id that does not parse) still renders the group's `not-found.tsx` on the client after Next.js's bare error shell; the dark `color-scheme` in the root viewport keeps that shell from flashing white.
