...
| Record pages   | `lib/cacheWindow`             | Cached renders (on-demand ISR) with no `loading.tsx`: a boundary gets no params, so a translated one reads the request headers and breaks the cache (`record-route-caching.test.ts`). Each route keeps its render for the window its record calls for (`CACHE_WINDOW`: final, live, pending). A record that does not exist is the page's own not-found state, rendered on the server (a segment's `notFound()` reaches the browser as the bare error shell); a malformed id is the global 404 from `proxy.ts` (`lib/paramRoutes`). While the browser reads what the server could not, a page draws its own loading state in its final layout; a Signature's page uses `NFTDetailSkeleton` (`components/nft`). |
