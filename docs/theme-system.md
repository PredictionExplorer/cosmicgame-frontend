...
glass treatment saved in `e58da429` (the same style sources as `b675472`): the blue
canvas, white text, light-blue secondary, saturated violet accent, original brand
gradients/highlights, and glass controls with white-label cyan-to-violet buttons.
...
[Original Glass mobile](images/liquid-glass-mobile.png).
...
Original Glass keeps the backdrop sharp: saturation and contrast, without blur.
The original horizontal CTA stops are `#06AEEC`, `#35C9FF`, and `#9C37FD`, with
white labels and the saved gloss. Reduced motion prevents control movement; reduced
