  ...
  class {
    ...
    paint(ctx, size, properties) {
      ...
      const cc = function (x, y) {
        if (typeof x !== 'string' || typeof y !== 'string') return [0, 0];
        var fx = 0,
          fy = 0;
        if (x.indexOf('calc') > -1) {
      ...
      for (i = 0; i < Cpoints.length - 1; i++) {
        ...
        var distance = Math.min(
          Math.sqrt(
            (Cpoints[i + 1][1] - Ppoints[i][1]) ** 2 + (Cpoints[i + 1][0] - Ppoints[i][0]) ** 2,
          ),
          Math.sqrt((Cpoints[i][1] - Ppoints[i][1]) ** 2 + (Cpoints[i][0] - Ppoints[i][0]) ** 2),
        );
        rr = Math.min(distance * Math.tan(angle / 2), Radius[i]);
        ctx.arcTo(Ppoints[i][0], Ppoints[i][1], Cpoints[i + 1][0], Cpoints[i + 1][1], rr);
      }
      var angle =
        Math.atan2(Cpoints[0][1] - Ppoints[i][1], Cpoints[0][0] - Ppoints[i][0]) -
        Math.atan2(Cpoints[i][1] - Ppoints[i][1], Cpoints[i][0] - Ppoints[i][0]);
      if (angle < 0) {
        angle += 2 * Math.PI;
      }
      ...
      var distance = Math.min(
        Math.sqrt((Cpoints[0][1] - Ppoints[i][1]) ** 2 + (Cpoints[0][0] - Ppoints[i][0]) ** 2),
        Math.sqrt((Cpoints[i][1] - Ppoints[i][1]) ** 2 + (Cpoints[i][0] - Ppoints[i][0]) ** 2),
      );
      rr = Math.min(distance * Math.tan(angle / 2), Radius[i]);
