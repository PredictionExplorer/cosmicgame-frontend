...
export function nextSteps(locale: string, names = languageNames(locale)): readonly string[] {
  return [
    ...
    `Typography: if Clash Display / Inter lack the script, add a companion face (lib/fonts.ts) with an html:lang(${locale}) rule in styles/global.css, and an OG subset via npm run og:fonts (docs/i18n/README.md §5).`,
